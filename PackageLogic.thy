...
lemma defined_sum_move:
  assumes "a ## b"
    ...
    shows "a' ## y"
  by (metis assms(1) assms(2) assms(3) defined_def sep_algebra.asso1 sep_algebra_axioms)
    ...
    proof -
      ...
      proof (rule package_satI)
        ...
        proof (cases "b |a'|")
          ...
          then show ?thesis
            by (metis \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u\<close> package_logic.package_sat_def package_logic_axioms sat.simps(2))
        next
          case False
